...
#Visualización con ggplot2
ggplot(data.frame(pca1@scores), aes(x=PC1, y=PC2, color=datos_mirna$respuesta)) + 
...
scatter3d(pca1@scores[,1], pca1@scores[,2], pca1@scores[,3], 
               group=datos_mirna$respuesta, ellipsoid=TRUE, surface=FALSE)

#t-distributed stochastic neighbor embedding (t-SNE)
library(Rtsne)
tsne1 <- Rtsne(datos_mirna[,14:6622], perplexity=4, theta=0.0, max_iter=10000)
plot(tsne1$Y, col=datos_mirna$respuesta, pch=16, las=1)
